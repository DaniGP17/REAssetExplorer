// Alpha GBuffer Pixel Shader
// For materials with alpha transparency (Env_Alpha)
// Includes alpha testing for transparent materials
...
cbuffer UserMaterial : register(b2)
{
    ...
    float  AlphaTestRef;            // Referencia para alpha test (0.5 por defecto)
...
struct OutputStruct
{
    ...
    float4 Target3 : SV_Target3;    // RT3: AO + Screen UV + alpha
};
...
OutputStruct main(in InputStruct IN)
{
    ...
    float4 atos = AlphaTranslucentOcclusionSSSMap.Sample(AutomaticWrap, atosUV);
    
    // Alpha from ATOS texture (R channel)
    float alpha = atos.r;
    
    // Multiply by BaseColor.a for material control
    alpha *= BaseColor.a;
    
    // Alpha test - discard transparent pixels
    // AlphaTestRef is typically 0.5 for cutout materials
    ...
    // Encode normal using octahedron encoding
    ...
    // Output AO, screen UV and alpha
    OUT.Target3.x = ao;

    float2 screenUV = IN.Position.xy * screenInverseSize;
    OUT.Target3.yz = screenUV;
    OUT.Target3.w  = alpha;   // Write alpha to Target3.w for transparency handling
