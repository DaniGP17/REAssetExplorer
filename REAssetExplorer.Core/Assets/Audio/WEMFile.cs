 ...
 * Original file: WEMFile.cs
 *
 * Adaptation by: DanielGP
 *
 * License: GPL-3.0
 */
...
namespace REAssetExplorer.Core.Assets.Audio;
...
public class WEMFile
{
    ...
    /// </summary>
    public TimeSpan Duration => SampleRate > 0 && _sampleCount > 0
        ? TimeSpan.FromSeconds((double)_sampleCount / SampleRate)
        : TimeSpan.Zero;
    ...
    public void GenerateOGG(string fileLocation, string codebooksLocation)
    {
        if (string.IsNullOrEmpty(codebooksLocation))
        {
            throw new ArgumentException("The codebooks path is required", nameof(codebooksLocation));
        }

        if (!File.Exists(codebooksLocation))
        {
            throw new FileNotFoundException($"Codebooks file not found: {codebooksLocation}");
        }
        ...
        using (OggStream ogg = new OggStream(File.Create(fileLocation)))
        {
            ...
            // Block sizes in samples
            uint blockSize0 = (uint)(1 << this._blocksize0Pow);
            uint blockSize1 = (uint)(1 << this._blocksize1Pow);
            
            // The granule position must start negative to compensate for the overlap
            // In the first Vorbis packet, only blockSize0/4 useful samples are produced
            // due to the overlap window.
            long cumulativeGranulePosition = -(long)(blockSize0 / 2);
            ...
            {
                GenerateOGGHeader(ogg, codebooksLocation, false, false, ref modeBlockFlag, ref modeBits);
            }

            {
                ...
                while (offset < this._dataChunkOffset + this._dataChunkSize)
                {
                    ...
                    if (this._modPackets)
                    {
                        ...
                        bool currentBlockFlag = modeBlockFlag[modeNumber];
                        
                        uint currentBlockSize = currentBlockFlag ? blockSize1 : blockSize0;
                        uint previousBlockSize = previousBlockFlag ? blockSize1 : blockSize0;
                        uint samplesThisPacket = (currentBlockSize + previousBlockSize) / 4;
                        
                        cumulativeGranulePosition += samplesThisPacket;
                        ...
                        previousBlockFlag = currentBlockFlag;
                        ogg.BitWrite(remainder, (byte)(8 - modeBits));
                    }
                    else
                    {
                        uint samplesThisPacket = blockSize0;
                        cumulativeGranulePosition += samplesThisPacket;
                    ...
                    ulong granuleToWrite = cumulativeGranulePosition >= 0 
                        ? (ulong)cumulativeGranulePosition 
                        : 0;
                    ogg.SetGranule(granuleToWrite);
